...
param grutorPrefs{Grutors, Shifts};  # integer value from 2 - 0 depending on how grutor g indicated their preference for shift t
param studentsPerGrutor >= 0;
param maxShiftsPerGrutor >= 0;
param studentAttendanceOdds{Students, Shifts};
...
maximize Happiness:  # Want to maximize grutor happiness + student happiness + student&grutor happiness
	sum{t in Shifts} ((-1 * (numGrutorsSlack[t] + numGrutorsSurplus[t])) +
					   sum{g in Grutors} (grutorPrefs[g, t] * grutorWorking[g,t]) +  # multiply by 1/2 to normalize
...
subject to SPSUpperCalc{t in Shifts}:
 	studentDemand[t] >= sum{s in Students} studentAttendanceOdds[s, t];
	
subject to SPSLowerCalc{t in Shifts}:
	studentDemand[t] <= sum{s in Students} (studentAttendanceOdds[s, t]) + 1;
