...
subject to MaintainHappiness:  # Want to maintain certain level of happiness while 
	sum{t in Shifts} ((-1 * (numGrutorsSlack[t] + numGrutorsSurplus[t])) +  # We can normalize this by dividing by the number of students at the shift, need to make studentDemand a param though.
